...
function AppContent() {
        ...
        if (isLoggedIn && authUser) {
            ...
            if (storedUser && authenticated === "true") {
                try {
                    ...
                    setUserData({ customerId: p.customerId, consultantId: p.consultantId, email: p.email, name: p.name, role: normalizeRole(p.role) });
                    setIsAuthed(true);
                } catch {}
            }
        }
    ...
    const handleLoginSuccess = (user: { customerId: string; consultantId?: string; email: string; name: string; role?: string }) => {
        ...
        else                     { setActivePage("dashboard");   setPageHistory(["dashboard"]); }
    };

    const handleLogout = () => {
        setIsAuthed(false); setUserData(null); setConsultantId("");
        setActivePage("dashboard"); setPageHistory(["dashboard"]);
    ...
    const handleNavigation = (page: string) => {
        ...
        if (new Set(["dashboard", "projects", "listings", "client", "consultants", "reports", "hours", "settings", "profile", "messages"]).has(page) && !isStaff) return;
        if (page === "client_home" && !isClient) return;
        setActivePage(page);
        setPageHistory(p => [...p, page]);
    };
    ...
    const handleBack = () => {
        setPageHistory(p => {
            ...
            const h = [...p]; h.pop();
            setActivePage(h[h.length - 1] || "dashboard");
            return h;
        });
    };
        ...
        <div className={`min-h-screen ${isDark ? "bg-[#0a0a0a] text-white" : "bg-white text-gray-900"}`}>
            ...
            <div className={`min-h-screen transition-all duration-300 ${!sidebarToggle ? "ml-72" : "ml-0"}`}>
                <Navbar
                    ...
                    userEmail={userData?.email}
                />
                <main className="pt-16 px-4 md:px-6 pb-6" key={activePage}>
                    <div className="animate-fadeIn">{renderActivePage()}</div>
                </main>
            </div>
        </div>
    );
