...
const AdminTab: React.FC<AdminTabProps> = ({ onNavigate }) => {
    ...
    const fetchUsers = async () => { const d = await safeFetch(`${API_BASE}/api/orgs/me`); if (d?.data?.members) setUsers(d.data.members.map((u: User) => ({ ...u, status: u.status || "active", role: u.role || "client" }))); };    const fetchConsultants = async () => { const d = await safeFetch(`${API_BASE}/api/consultants`); if (d?.data) setConsultants(d.data.map((c: Consultant) => ({ ...c, status: c.status || "active" }))); };
