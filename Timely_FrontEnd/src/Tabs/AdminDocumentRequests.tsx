...
const AdminDocumentRequests: React.FC<Props> = ({ onNavigate, adminEmail = "admin@timely.com", adminName = "Admin" }) => {
    ...
    const loadData = async () => { setLoading(true); try { const r = await fetch(`/api/users-report`); if (r.ok) { const d = await r.json(); setUsers(d.data || []); } } catch {} setRequests(ReqAPI.getAll()); setUploads(UplAPI.getAll()); setLoading(false); };
    useEffect(() => { loadData(); }, []);
